...
def tfrecord_debugger(fn):
    ...
    with tf.Session() as sess:
        ...
        print("are these ones the correct dimensions? {}-{}".format(img.shape,
                                                                    label.shape))
    sess.close()
    return None
    ...
    with tf.python_io.TFRecordWriter(filename) as writer:
        for index, ex in enumerate(example_data):
            ...
            example = tf.train.Example(features=tf.train.Features(feature={
                ...
                'label': _int64_feature(int(_10_class_encoding(ex['label']))),
                # _encode_labels
...
def transform_preprocess_data(path, out_dir, name: str, channels=None,
    ...
    if name is 'train':
        data_files = [fn for fn in filenames if
                      os.path.basename(fn).split('_')[0] not in [test_persons,
                                                                 eval_persons]]
    #
    ...
    for idx, c in enumerate(tqdm(chunks)):
        ...
        tf_parts = [(k * samples_per_tf_record) for k in range(len(chunk_data))
...
def generate_records():
    ...
    transform_preprocess_data(trackings, out_dir, 'evaluation',
...
def _20190710_generate_records():
    ...
    evaluation = ['0fc8854f-c', '27089a2b-2', '0b7cf78c-1']
    test = ['d847614e-4', '2f3d5c91-2']
    transform_preprocess_data(trackings, out_dir, 'train', channels=3,
                              num_tfrecords=5)
    transform_preprocess_data(trackings, out_dir, 'test', test_persons=test,
...
def train_test_split_preprocess_data(path, out_dir, name: str = None,
    ...
    if name is 'evaluation' and eval_persons is not None:
        data_files = [fn for fn in filenames if
                      os.path.basename(fn).split('_')[0] in eval_persons]
        generate_examples(None, data_files, out_dir, channels, name, chunk_len,
                          num_tfrecords)
    else:
        data_files = [fn for fn in filenames if
                      os.path.basename(fn).split('_')[0] not in eval_persons]

        for activity in important_labels:
            activity_wise_data_files = [i for i in data_files if activity in i]

            seventy_percent = math.ceil(len(activity_wise_data_files) * 0.7)

            generate_examples(activity, activity_wise_data_files[
                                        :seventy_percent],
...
def generate_examples(activity, data_files, out_dir, channels, name,
                      chunk_len=50,
                      num_tfrecords=5):
    chunks = [data_files[k:k + chunk_len] for k in
              range(0, len(data_files), chunk_len)]
...
def _20190712_generate_records_10classes_train_test_split():
    """
    # ACCE AND GYRO DATA
    :return: REMEMBER TO ELIMINATE FROM THE FOLDER OF THE RECORDS, THE FILES WITH SMALL SIZE, THEY ARE NOT USEFUL
    ### THEY HAVE FEW EXAMPLES
    ...
    evaluation = ['0fc8854f-c', '27089a2b-2', '0b7cf78c-1']
    test = ['d847614e-4', '2f3d5c91-2']
    # train_test_split_preprocess_data(trackings, out_dir, '', )
    train_test_split_preprocess_data(trackings, out_dir,
                                     eval_persons=evaluation,
                                     channels=3, num_tfrecords=5)
    train_test_split_preprocess_data(trackings, out_dir,name='evaluation',
                                     eval_persons=evaluation,
                                     channels=3, num_tfrecords=5)
    return None
...
if __name__ == '__main__':
    ...
    _20190712_generate_records_10classes_train_test_split()
