...
class _sence_net(Net):
    ...
    def build_model(self):
        self.conv1_1 = self.conv('conv1_1', self.x, 32, kernel_size=[3, 3], stride=[1, 1, 1, 1])
        self.conv1_2 = self.conv('conv1_2', self.conv1_1, 32, kernel_size=[3, 3], stride=[1, 1, 1, 1])
        self.pool1 = self.pool('pool1', self.conv1_2, kernel=[1, 2, 2, 1], stride=[1, 2, 2, 1], is_max_pool=True)

        self.conv2_1 = self.conv('conv2_1', self.pool1, 64, kernel_size=[5, 5], stride=[1, 1, 1, 1])
        self.conv2_2 = self.conv('conv2_2', self.conv2_1, 64, kernel_size=[5, 5], stride=[1, 1, 1, 1])
        self.pool2 = self.pool('pool2', self.conv2_2, kernel=[1, 2, 2, 1], stride=[1, 2, 2, 1], is_max_pool=True)

        self.fc1 = self.fc('fc1', self.pool2, out_nodes=128)
        ...
        self.fc2 = self.fc('fc2', self.batch_norm1, out_nodes=256)
        self.batch_norm2 = self.batch_normalization('batch_norm2', self.fc2, training=self.is_training)

        self.fc3 = self.fc('fc3', self.batch_norm2, out_nodes=512)
        self.dropout3 = self.dropout(self.fc3, 0.25)
        self.logits = self.fc('fc8', self.dropout3, out_nodes=self.config.n_classes, ReLu=False)
